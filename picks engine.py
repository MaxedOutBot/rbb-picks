...
def determine_pick(game, opener_snap, ml_delta, juice_flip, j75,
    ...
    # GATE 4: Run Line (spread) — when market score is decent but no clear total/ML signal
    # Home team run line (-1.5) if home win probability is strong
    if home_dv_c >= 0.60 and market_score >= 55:
        return {
            "type": "RUNLINE",
            "bet": f"{home} -1.5",
            "side": "home_rl",
            "release": market_score >= 60,
            "tag": "NO TAG",
            "track": "TRACK 2",
        }
    if home_dv_c <= 0.40 and market_score >= 55:
        return {
            "type": "RUNLINE",
            "bet": f"{away} +1.5",
            "side": "away_rl",
    ...
    # GATE 5: Soft ML — lower confidence but still releasable
    if home_dv_c >= 0.58 and market_score >= 50:
        return {
            "type": "ML",
            "bet": f"{home} ML",
            "side": "home",
    ...
    if home_dv_c <= 0.42 and market_score >= 50:
        return {
            "type": "ML",
            "bet": f"{away} ML",
            "side": "away",
...
def compute_units(stars, tag):
    """Unit size based on stars. Returns numeric float for tracking."""
    if tag == "BLOCKED": return 0
    return STARS_TO_UNITS.get(stars, 1.0)

def units_display(units_float):
    """Human-readable unit label for public display."""
    if units_float == 3.0: return "3 Units"
    if units_float == 2.0: return "2 Units"
    if units_float == 1.5: return "1.5 Units"
    if units_float == 1.0: return "1 Unit"
    if units_float == 0.5: return "0.5 Units"
    return f"{units_float} Units"
...
def analyze_slate(current_odds, opener_snapshot, pitchers, il_players, transactions):
    ...
    for game in current_odds:
        ...
        picks.append({
            ...
            "stars": stars,
            "units": units,
            "units_display": units_display(units),
...
def main():
    ...
    # ── Ensure minimum 3 picks per day ──
    # If fewer than 3 released, promote the next best non-blocked picks
    MIN_PICKS = 3
    if len(released) < MIN_PICKS:
        not_released = sorted(
            [p for p in all_picks if not p["released"] and p["tag"] != "BLOCKED"],
            key=lambda x: x["market_score"] + x["baseball_score"] + x["situation_score"],
            reverse=True
        )
        needed = MIN_PICKS - len(released)
        for p in not_released[:needed]:
            p["released"] = True
            # Lower star rating for promoted picks
            p["stars"] = max(1, p["stars"] - 1)
            p["units"] = STARS_TO_UNITS.get(p["stars"], 0.5)
            p["units_display"] = units_display(p["units"])
            p["tag"] = p.get("tag", "NO TAG")
            released.append(p)
        pt(f"Promoted {min(needed, len(not_released))} picks to reach minimum of {MIN_PICKS}")

    # Sort released by combined score
    released.sort(
        key=lambda x: x["market_score"] + x["baseball_score"] + x["situation_score"],
        reverse=True
    )
    pt(f"Final released picks: {len(released)}")

    # Best play = top released pick
    best_play = released[0] if released else None

    # Free pick = lowest-scored released pick (save best for paid)
    # Subscribers get ALL picks including the free one
    free_pick = None
    if released:
        free_pick = released[-1]  # weakest pick goes free — best are premium
        # Mark free pick so website knows which one to show publicly
        free_pick["is_free"] = True

    # Premium picks = ALL released picks (including free one for paid subscribers)
    # Paid subscribers see the complete card including the free pick
    for p in released:
        p["is_free"] = (p is free_pick)

    # Premium picks = ALL released picks (paid subscribers see everything including free pick)
    premium_picks = released  # full card — free pick is included
    non_free_picks = [p for p in released if not p.get("is_free")]
    ...
    output = {
        ...
        "premium_picks": premium_picks,   # ALL picks for paid subscribers
        "best_play": best_play,
        "free_pick": free_pick,
    ...
    # Append to picks_log.json — permanent record for units tracking
    # This is read by the tracker pages to auto-populate units
    if free_pick:
        log_entry = {
            "date": date.today().strftime("%m-%d-%y"),
            "date_iso": date.today().isoformat(),
            "game": free_pick.get("game", ""),
            "bet": free_pick.get("bet", ""),
            "odds": free_pick.get("odds", ""),
            "stars": free_pick.get("stars", 0),
            "units": free_pick.get("units", 1.0),
        ...
        try:
            ...
            pass
        # Only append if this date isn't already logged
        existing_dates = {e.get("date") for e in existing_log}
        if log_entry["date"] not in existing_dates:
            existing_log.append(log_entry)
            with open("picks_log.json", "w") as f:
                json.dump(existing_log, f, indent=2)
            pt(f"picks_log.json updated — {len(existing_log)} total entries")
        else:
            # Update existing entry for today (in case odds/stars changed)
            for i, e in enumerate(existing_log):
                if e.get("date") == log_entry["date"]:
                    existing_log[i] = log_entry
                    break
            with open("picks_log.json", "w") as f:
                json.dump(existing_log, f, indent=2)
            pt("picks_log.json — updated today's entry")
