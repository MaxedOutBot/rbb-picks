...
def pick(g):
    ...
    bk = next((b for b in g.get("bookmakers", [])
                if b["key"] in ["draftkings", "fanduel"]), None)
    if not bk:
        return None
    ...
    ap = 1 - hp
    if hp >= 0.55:
        bet, o, sp = home + " ML", ho, hp
    elif ap >= 0.55:
        bet, o, sp = away + " ML", ao, ap
    else:
        return None
    # Skip extreme chalk — no value
    if float(str(o).replace("+", "")) < -250:
        return None
    stars = 5 if sp >= 0.65 else 4 if sp >= 0.62 else 3 if sp >= 0.58 else 2
    units = STARS_TO_UNITS.get(stars, 1.0)
    sig = [
        "Sharp money analysis complete - model found value at current line",
    ...
    return {
        ...
        "stars": stars,
        "units": units,
        "units_display": str(units) + " Units",
        "signals": sig,
        "market_score": int((sp - 0.5) * 300),
        "released": True,
        "is_free": False,
        "tag": "NO TAG"
    }
...
out = {
    "date": today,
    "generated_at_pt": now.strftime("%B %d, %Y %I:%M %p PT"),
    "last_updated": now.strftime("%I:%M %p PT"),
    "sport": "MLB",
...
print("Saved", len(picks), "picks for upcoming games only")
if fp:
    try:
